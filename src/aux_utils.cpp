...
namespace diskann {
  ...
  T *load_warmup(const std::string &cache_warmup_file, uint64_t &warmup_num,
    ...
    if (file_exists(cache_warmup_file)) {
      diskann::load_aligned_bin<T>(cache_warmup_file, warmup, (size_t&)warmup_num,
                                   file_dim, file_aligned_dim);
      if (file_dim != warmup_dim || file_aligned_dim != warmup_aligned_dim) {
        std::stringstream stream;
        stream << "Mismatched dimensions in sample file. file_dim = "
  ...
  int merge_shards(const std::string &vamana_prefix,
    ...
    for (const auto &id_shard : node_shard) {
      ...
      if (cur_id < node_id) {
        ...
        nnbrs =
            (unsigned) (std::min<uint64_t>)(final_nhood.size(), (uint64_t) max_degree);
    ...
    // Gopal. random_shuffle() is deprecated.
    std::shuffle(final_nhood.begin(), final_nhood.end(), urng);
    nnbrs = (unsigned) (std::min<uint64_t>)(final_nhood.size(), (uint64_t) max_degree);
    // write into merged ofstream
    diskann_writer.write((char *) &nnbrs, sizeof(unsigned));
...
template<typename T>
int search_disk_index(const std::string index_prefix_path, const diskann::Metric metric, const std::string data_type, const std::string query_bin, 
                        const std::string result_output_prefix, const char* indexBuildParameters, uint32_t num_results) {
  // load query bin
  ...
  uint64_t num_nodes_to_cache = std::atoi(param_list[0].c_str());
  uint32_t num_threads = std::atoi(param_list[1].c_str());
  uint32_t beamwidth = std::atoi(param_list[2].c_str());
  _u64 L = std::atoi(param_list[3].c_str());
  ...
  diskann::cout << "********* Loaded query binary file..." << std::endl;
  ...
  diskann::cout << "*********** Loaded Flash Index..." << std::endl;
  if (res != 0) {
    return res;
  }
  // cache bfs levels
  ...
  diskann::cout << "************ Loaded query cache from sample queries" <<std::endl;
  node_list.clear();
...
#pragma omp parallel for schedule(dynamic, 1)
  ...
  template DISKANN_DLLEXPORT int search_disk_index<int8_t>(
      const std::string index_prefix_path, diskann::Metric compareMetric, 
      const std::string data_type, const std::string query_bin, 
      const std::string result_output_prefix, const char *indexBuildParameters,
      uint32_t num_neighbours);


  template DISKANN_DLLEXPORT int search_disk_index<uint8_t>(
      const std::string index_prefix_path, diskann::Metric compareMetric, 
      const std::string data_type, const std::string query_bin, 
      const std::string result_output_prefix, const char *indexBuildParameters,
      uint32_t num_neighbours);
  ...
  template DISKANN_DLLEXPORT int search_disk_index<float>(
      const std::string index_prefix_path, diskann::Metric compareMetric, 
      const std::string data_type, const std::string query_bin, 
      const std::string result_output_prefix, const char *indexBuildParameters,
      uint32_t num_neighbours);
